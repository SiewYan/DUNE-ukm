...
using VecII_t = const ROOT::RVec<RVec<int>>&;
ROOT::RVec<int> getMatchIdx( VecI_t recoID , VecII_t truthID ){
...
df = df.Filter( 'pdg == abs(13)' , 'look at muon' ) 
...
df = df.Define( 'dx' , 'EndPointx_tpcAV - EndPointx' )\
       ...
       .Define( 's' , 'dx+dy+dz' )\
       .Filter( 's < 1e-10' , 'ensure length is within 1e-10' )

# get the indices of reco track where G4 TrackID match the MC's
df =df.Define( 'matchedMCIdx' , 'getMatchIdx( TrackId , trkidtruth_pandoraTrack )' )
